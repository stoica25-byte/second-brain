...
- **Optimización de Contexto de Antigravity (Modo Stateless y Marcadores Semánticos)**: Se actualizó el archivo de instrucciones globales `GEMINI.md` implementando el protocolo de tokens efímeros para proyectos medianos y grandes. Se incorporó el sistema de marcadores semánticos en código (`// @section` / `// @end`) con mapas de proyecto `.md` de Obsidian, el soporte de handoffs atómicos nombrados por proyecto (`handoff-[proyecto].md`), protección contra loops infinitos de handoff (límite de 2 intentos), delegación proactiva a subagentes especialistas, y flexibilidad para la re-apertura de archivos y consulta del Vault ante sub-errores o debugging.
