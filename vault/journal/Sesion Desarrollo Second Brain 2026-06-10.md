...
- **Optimización de Contexto de Antigravity (Modo Stateless y Marcadores Semánticos)**: Se actualizó el archivo de instrucciones globales `GEMINI.md` implementando el protocolo de tokens efímeros para proyectos medianos y grandes. Se incorporó el sistema de marcadores semánticos en código (`// @section` / `// @end`) compatible con formateadores y linters, con mapas de proyecto y compartidos en `.md` de Obsidian, soporte de handoffs atómicos nombrados por proyecto (`handoff-[proyecto].md`) con límite de tamaño de nota, gestión segura de ramas de Git temporales para evitar subir código roto a master/main, protección contra loops infinitos de handoff (límite de 2 intentos con parada rígida), procesamiento Bottom-Up en `multi_replace` para evitar desalineación de rangos, delegación proactiva a subagentes especialistas, y flexibilidad para la re-apertura de archivos y consulta del Vault ante sub-errores o debugging.
